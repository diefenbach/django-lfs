...
def separate_order_delivery_times(apps, schema_editor):
    ...
    for old_odt in OrderDeliveryTime.objects.all():
        ...
        order = old_odt.order
        DeliveryTimeOrder.objects.get_or_create(
            order=order,
            defaults=dict(min=delivery.min, max=delivery.max, unit=delivery.unit, description=delivery.description),
        )
        delivery.delete()
...
def concat_order_delivery_times(apps, schema_editor):
    ...
    for new_odt in DeliveryTimeOrder.objects.all():
        OrderDeliveryTime.objects.get_or_create(
            order=new_odt.order,
            defaults=dict(min=new_odt.min, max=new_odt.max, unit=new_odt.unit, description=new_odt.description),
        )
        new_odt.delete()
            ...
            fields=[
                ...
                (
                    ...
                    models.OneToOneField(
                        related_name="delivery_time", verbose_name="Order", to="order.Order", on_delete=models.CASCADE
                    ),
                ),
            ],
            options={
