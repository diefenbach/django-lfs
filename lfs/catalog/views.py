...
def select_variant(request):
    ...
    msg = _("The product has been changed according to your selection.")
...
def calculate_packing(
    request, id, quantity=None, with_properties=False, as_string=False, template_name="lfs/catalog/packing_result.html"
):
...
def select_variant_from_properties(request):
    ...
    if variant is None:
        msg = _("The choosen combination of properties is not deliverable.")
...
def set_filter_form(request):
    ...
    for key in req.keys():
        values = req.getlist(key)
        if key.startswith("property-"):
            property_group_id, property_id = key.split("|")
            property_group_id = property_group_id.replace("property-", "")

            new_key = "{0}_{1}".format(property_group_id, property_id)
            product_filter["select-filter"][new_key] = {
                "value": "|".join(values),
                "property_id": property_id,
                "property_group_id": property_group_id,
            }
    ...
    url = reverse("lfs_category", kwargs={"slug": req.get("category_slug")})
    return HttpResponseRedirect(url)
...
def category_products(request, slug, start=1, template_name="lfs/catalog/categories/product/default.html"):
    ...
    pagination_data["total_text"] = __("%(count)d product", "%(count)d products", amount_of_products) % {
        ...
        for property_dict in product.get_configurable_properties():
            ...
            try:
                ppv = ProductPropertyValue.objects.get(
                    product=product, property_group=property_group, property=prop, type=PROPERTY_VALUE_TYPE_DEFAULT
                )
                ppv_value = ppv.value
    ...
    if (request.POST if request.method == "POST" else request.GET).get("add-to-cart") is not None:
        ...
        if variant is None:
            ...
            return lfs.core.utils.set_message_cookie(
                variant.get_absolute_url(), msg=_("The choosen combination of properties is not deliverable.")
            )
