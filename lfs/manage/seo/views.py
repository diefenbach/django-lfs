...
class SEOView(View):
    ...
    def get_unique_klass_name(model_klass):
        klass_name = "{0}_{1}".format(model_klass.__module__, model_klass.__name__.lower())
    ...
    def get_seo_urlpattern(cls, model_klass, form_klass=None, template_name="manage/seo/seo.html"):
        ...
        view_obj = cls.as_view(form_klass=form_klass, model_klass=model_klass, template_name=template_name)
        # lfs.manage.seo.views
        return (
            re_path(
                r"^manage-seo/%s/(?P<id>\d*)/$" % klass_name,
    ...
    def __init__(self, model_klass, form_klass=None, template_name="manage/seo/seo.html", *args, **kwargs):
        ...
        if not form_k:
            ...
            form_k = modelform_factory(model_klass, fields=("meta_title", "meta_keywords", "meta_description"))
    ...
    def render_to_response(self, form, message=""):
        ...
        return HttpResponse(
            json.dumps({"seo": seo_html, "message": message}, cls=LazyEncoder), content_type="application/json"
        )
