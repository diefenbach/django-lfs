// Topseller Products Management JavaScript
...
function initializeSortable() {
    ...
    if (table && !sortableTable) {
        sortableTable = Sortable.create(table, {
            ...
            handle: ".handle",
            onEnd: function(evt) {
                // Get the new order of topseller items
                const rows = table.querySelectorAll('tr[data-id]');
                const topsellerIds = Array.from(rows).map(row => row.getAttribute('data-id'));
                
                // Send AJAX request to update positions
                const sortUrl = document.querySelector('[data-sort-url]')?.getAttribute('data-sort-url') || '/manage/sort-topseller';
                fetch(sortUrl, {
                    method: 'POST',
                    headers: {
...
document.addEventListener('DOMContentLoaded', function() {
    new CheckboxManager('.select-all-topseller', '.select-topseller');
