...
class Manufacturer(models.Model):
    ...
    def get_format_info(self):
        ...
        if self.active_formats is True:
            return {"product_cols": self.product_cols, "product_rows": self.product_rows}
        else:
            try:
                ...
                return {"product_cols": 3, "product_rows": 3}
            else:
                return {"product_cols": shop.product_cols, "product_rows": shop.product_rows}
    ...
    def get_filtered_products(self, filters, price_filter, sorting):
        ...
        if filters:
            ...
            cursor.execute(
                ...
                GROUP BY product_id
                HAVING count(*)=%s"""
                % (product_ids, fstr, PROPERTY_VALUE_TYPE_FILTER, len(filters))
            )
            ...
            if all_variants:
                ...
                cursor.execute(
                    ...
                    GROUP BY product_id
                    HAVING count(*)=%s"""
                    % (all_variant_ids, fstr, PROPERTY_VALUE_TYPE_FILTER, len(filters))
                )
